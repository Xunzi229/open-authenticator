...
fn parse_migration(uri: &str) -> Result<Vec<QrAccount>, String> {
    ...
    while pos < decoded.len() {
        let Some((tag, p)) = read_varint(&decoded, pos) else { break };
        pos = p;
        let field = tag >> 3;
        let wire = tag & 7;
        if wire == 0 {
            let Some((_, p)) = read_varint(&decoded, pos) else { break };
            pos = p;
        } else if wire == 2 {
            let Some((len, p)) = read_varint(&decoded, pos) else { break };
            pos = p;
...
fn parse_otp(data: &[u8]) -> QrAccount {
    let mut pos = 0usize;
    ...
    let mut digits = 1u32;
    while pos < data.len() {
        let Some((tag, p)) = read_varint(data, pos) else { break };
        pos = p;
        let field = tag >> 3;
        let wire = tag & 7;
        if wire == 0 {
            let Some((v, p)) = read_varint(data, pos) else { break };
            pos = p;
            if field == 4 {
                algorithm = v as u32;
            }
            ...
            let Some((len, p)) = read_varint(data, pos) else { break };
            pos = p;
            if pos + len > data.len() {
                break;
            }
...
pub fn decode_image(bytes: &[u8]) -> Result<Vec<QrAccount>, String> {
    let img = image::load_from_memory(bytes).map_err(|_| "无法读取图片")?.to_luma8();
    ...
    let mut accounts = Vec::new();
    for grid in grids {
        let (_meta, content) = grid.decode().map_err(|_| "二维码无法解码")?;
        accounts.extend(parse_uri(&content)?);
    }
    Ok(accounts)
}
